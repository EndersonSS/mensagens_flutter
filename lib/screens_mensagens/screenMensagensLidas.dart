...
class ScreenMensagensLidas extends StatefulWidget {
  @override
  _ScreenMensagensLidasState createState() => _ScreenMensagensLidasState();
}

class _ScreenMensagensLidasState extends State<ScreenMensagensLidas> {
  ...
  future() {
    Provider.of<MensagensProvider>(context, listen: false)
        ...
        .then((value) {
      if (value != null) {
        List<Mensagens> streamMensagensLidas = value;
        streamController.add(streamMensagensLidas);
      } else {
        List<Mensagens> streamMensagensLidas = [];
        streamController.add(streamMensagensLidas);
      }
    });
  }
                      ...
                      onPressed: () {
                        setState(() {
                          if (isCheckd = !isCheckd) {
                            for (int i = 0;
                                i < msg.mensagensLidas.length;
                                i++) {
                              msg.mensagensLidas[i].isCheck = true;
                            }
                          } else {
                            for (int i = 0;
                                i < msg.mensagensLidas.length;
                                i++) {
                              msg.mensagensLidas[i].isCheck = false;
                            }
                ...
                child: StreamBuilder(
                  ...
                  builder:
                    ...
                    if (snapshot.data.isEmpty) {
                      return Center(child: Text("Sem Mensagens Lidas"));
                    } else {
                      return ListView.builder(
                          ...
                          itemBuilder: (BuildContext context, int index) {
                            ...
                            msg.mensagensLidas = item.mensagens;
                            return item.mensagens.isEmpty
                                ? Container(
                                    height: screenHeight * 1,
                                    child: Center(
                                        child: Text("Sem Mensagens Lidas")))
                                : Container(
                                    height: screenHeight * 1,
  ...
  apagandoMensagem() async {
    ...
    msg.alterarStatusMensagensLidas(2).then((value) {
  ...
  arquivandoMensagem() async {
    ...
    msg.alterarStatusMensagensLidas(3).then((value) {
      DialogBuilder(context).hideOpenDialog();
      if (value == false) {
        return showDialog<bool>(
          context: context,
  ...
  void addListaSelelcionados() {
    ...
    setState(() {
      var contain =
          msg.mensagensLidas.where((element) => element.isCheck == true);
      if (contain.isEmpty) {
        visibleOpcoesAppBar = false;
        isCheckd = false;
      }
      ...
      if (contain.length == msg.mensagensLidas.length) {
        isCheckd = true;
      }
    });
  }
